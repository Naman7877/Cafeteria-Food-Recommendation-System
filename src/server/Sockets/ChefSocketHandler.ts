...
export const giveFeedback = async (socket: Socket, connection: any, data: any) => {
	try {
...
export const getRecommendation = async (socket: Socket, connection: any, data: any) => {
	try {
	...
	try {
		...
		if (maxVoteItem.length > 0) {
			...
			if (existingFinalMenuItems.length > 0) {
				console.log(`Item has already been added to the final_menu for '${mealTime}' for today.`);
